...
export const ProductsProvider = ({ children }) => {
    ...
    const fetchProductImages = async (id) => {
        ...
        try {
            const imagesProductUrl = `http://localhost:8080/images/${id}`;
            const response = await axios.get(imagesProductUrl);
            const singleProductImages = response.data;
            dispatch({
                type: GET_SINGLE_PRODUCT_IMAGES_SUCCESS,
                payload: singleProductImages,
            });
        } catch (error) {
    ...
    const fetchProductQuantity = async (id) => {
        ...
        try {
            const productStockUrl = `http://localhost:8080/inventories/${id}`;
